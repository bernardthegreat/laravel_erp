      ...
      <table id="example2" class="table table-bordered table-striped text-center">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              <div class="btn-group">
                <a class="btn btn-danger btn-sm process_payment_btn" href="#" data-dr-number="{{$sale->dr_no ?? ''}}"
                  data-invoice-number="{{$sale->invoice_no ?? ''}}" data-url="{{ route('process_payment') }}"
                  data-toggle="modal" data-target="#modal-default" data-placement="top" data-placement="top"
                  rel="tooltip" title="Mark as Paid DR # {{$sale->invoice_no}}" data-original-title="Edit">
    ...
    <div class="card-body">
      <table id="example3" class="table table-bordered table-striped text-center">
        ...
        <tbody>
          ...
          <tr>
            <td> {{ date('m/d/Y h:i:s A', strtotime($sale->created_at)) }} </td>
            <td> {{ date('m/d/Y h:i:s A', strtotime($sale->paid_on)) }} </td>
            <td> {{$sale->client_name}} </td>
            <td> {{$sale->item_name}} </td>
            <td> {{$sale->dr_no}} </td>
            <td> {{$sale->remarks}} </td>
